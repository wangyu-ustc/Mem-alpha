...
class MemoryGenerationManager:
    ...
    def __init__(
        ...
        with open("/mnt/project/wangyu/work/Mem-alpha/config/prompts_wrt_datasource.yaml", "r") as f:
    ...
    def _process_chunk_with_memory_operations(self, rollings: DataProto, current_chunks: List[str], batch_memories: List[Memory]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        memory_agent_template = MemoryAgent(agent_config={'model_name': '/mnt/project/wangyu/models/Qwen3-4B', 'vllm': False, 'including_core': self.config.including_core}, is_template=True)
